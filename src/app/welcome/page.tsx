...
import { DonationDialog } from '../UI/dialog/donation/donation-dialog';
import { shouldShowDonationDialog, updateDonationDialogShown } from '../utils/donation';
        ...
        <>
            <div className={`text-center flex justify-center items-center flex-col h-full w-full ${isMounted ? 'opacity-100 transform scale-100 transition-all duration-1000' : 'opacity-0 transform scale-95'}`}>
                ...
                <Link legacyBehavior href="/choose">
                    <a><HighlightButton text="Get Started" loading={false}></HighlightButton></a>
                </Link>

                {!isTauri &&
                    <div className='fixed flex-col hidden xl:flex p-4 justify-end items-end bottom-4 right-0'>
                        <div className='rounded-[20px] w-fit flex justify-center items-center aspect-square scale-75 border-tertiary border relative'>
                            <QRGen link={handleDownload}></QRGen>
                        </div>
                        <div className='flex justify-end items-center mr-9 -mt-4 w-[440px]'>
                            <DownloadButton handleFunction={handleDownload} text="Download"></DownloadButton>
                        </div>
                    </div>
                }
            </div>
            ...
            <DonationDialog 
                ...
                onOpenChange={handleDonationDialogClose} 
            />
        </>
    );
}
